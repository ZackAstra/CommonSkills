...
description_cn: 基于 21 种布局和 22 种视觉风格，生成专业信息图：先分析内容结构和受众，再推荐布局和风格组合，确认后生成图片。适用于信息图、流程说明、对比图、导购图和高密度信息大图等场景。
...
**鉀?Never substitute SVG, HTML, canvas, or other code-based rendering for raster image generation.** If the raster image tool is unavailable or fails after retry, ask the user how to proceed 鈥?do **not** silently emit SVG, write inline `<svg>` markup, or produce HTML/CSS art as a substitute. This applies even if the article/section seems "diagram-like": this skill is for generated bitmap infographics.

**鉀?Never repair rendered text by painting over a generated bitmap.** Do not use ImageMagick, Pillow, Canvas, SVG, HTML/CSS, OCR scripts, or any other programmatic overlay to cover, rewrite, erase, stroke, or replace labels, headings, callouts, data values, or any other text inside an already generated infographic. If text is wrong or unclear, regenerate from a corrected prompt, switch to a layout with less on-image text, or ask the user which imperfect candidate to keep.
...
- Pasted image with no path 鈫?ask the user for the path, or extract style traits verbally as a text fallback
...
- If `usage: direct` 鈫?use `ImageGenWithRef` and pass references according to its runtime tool schema
...
- Skip confirmation only when the current request explicitly says to do so, for example: `--no-confirm`, "鐩存帴鐢熸垚", "涓嶇敤纭", "璺宠繃纭", "鎸夐粯璁ゅ嚭鍥?, or equivalent wording.
...
Default combination: `bento-grid` + `craft-handmade` (fallback recommendation only 鈥?per the [Confirmation Policy](#confirmation-policy), defaults never bypass Step 4).
...
|--------------|--------|--------------------|----------------|--------------|
| 楂樺瘑搴︿俊鎭ぇ鍥?/ high-density-info | `dense-modules` | `morandi-journal`, `pop-laboratory`, `retro-pop-grid`, `retro-popup-pop` | portrait | 鈥?|
...
**Hard gate**: this step is mandatory per the [Confirmation Policy](#confirmation-policy) 鈥?Steps 5鈥? cannot start until the user confirms here (or explicitly opts out with `--no-confirm` / equivalent in the current request).
...
1. Ensure the full final prompt is persisted at `prompts/infographic.md` (already written in Step 5) BEFORE invoking the image tool 鈥?the file is the reproducibility record.
...
- **Edit directly** 鈥?open EXTEND.md and change fields. Full schema: `references/config/preferences-schema.md`.
- **Reconfigure interactively** 鈥?delete EXTEND.md (or ask "reconfigure infographic preferences" / "閲嶆柊閰嶇疆"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `preferred_layout: dense-modules`, `preferred_style: morandi-journal`, `preferred_aspect: portrait`, `language: zh` 鈥?shift the Step-3 recommendations and Step-4 defaults (per [Confirmation Policy](#confirmation-policy), these never bypass Step 4).
