...
- **Kimi**（Work / Chat 模式、Kimi Code IDE 插件）
- **Cursor**
- **Codex**
- **Trae**
- **Qoder**
...
├── .gitignore                   # Git 忽略规则
├── scripts/
│   └── sync-to-kimi.sh          # Kimi Work/Chat 复制同步脚本
...
当前共有 **64 个公共技能**，涵盖：写作、代码开发、数据分析、投研、办公自动化、PPT 生成等。
...
### 通道 B：Native Directory Copy（原生目录复制）—— 适用于 Kimi Work / Chat 模式

**Kimi 桌面客户端（Work / Chat 模式）的 `daimon` 内核在扫描技能目录时，会跳过 Symbolic Link（软链接）**。因此必须采用**原生目录复制**方式：

| Agent 程序 | 复制目标路径 | 来源 |
|-----------|-----------|------|
| Kimi Work / Chat | `~/AppData/Roaming/kimi-desktop/daimon-share/daimon/skills/<skill-name>` | `~/CommonSkills/<skill-name>` |
...
通过 Git Hook 实现：当 `CommonSkills` 仓库更新（pull / merge / checkout）时，**自动触发同步脚本**，确保 Kimi Work / Chat 的技能目录始终与仓库最新版本一致。

### 已配置的 Hook

| Hook 类型 | 触发时机 | 作用 |
|----------|---------|------|
| `post-merge` | 执行 `git pull` 或 `git merge` 后 | 同步最新技能到 Kimi Work/Chat |
| `post-checkout` | 执行 `git checkout` 切换分支后 | 同步当前分支技能到 Kimi Work/Chat |
| `post-commit` | 执行 `git commit` 后 | 可选：同步本地修改到 Kimi Work/Chat |
...
|-------|---------|---------|------------|
| **Kimi Work/Chat** | `~/AppData/Roaming/kimi-desktop/daimon-share/daimon/skills/` | 复制（原生目录） | ✅ 需要重启客户端 |
...
**结论**：只有**原生目录**（非软链接）才能被 Kimi Work/Chat 加载。

### 2. 配置差异

Kimi 客户端 Work 模式与 Kimi Code IDE 插件使用**不同的配置文件**：
- Kimi Code IDE 插件：`~/.kimi/config.toml`（支持 `extra_skill_dirs`）
- Kimi Work/Chat：`daimon-share/daimon/runtime/kimi-code/config.toml`（`extra_skill_dirs` 可能无效）
