...
name: scheduler
description: 定时任务管理：创建、查看、修改、删除、运行定时任务。用户提�?每天/每周/定时/提醒/每隔"�?明天下午3�?一次性任�?�?查看所有任�?删除任务"时使用。支�?cron 和一次性任务。关键词：每天、每隔、定时、提醒、任务、自动化、schedule、cron、task、reminder、once、job�?name_cn: "定时任务"
...
|------------|----------|-------------------|
| **Create Task** | "每天", "每隔", date, "提醒�?, "帮我创建定时任务" | [add-task.md](references/add-task.md) |
| **View Tasks** | "查看", "显示", "所有任�?, "任务列表" | [list-tasks.md](references/list-tasks.md) |
| **Run Task** | "立即运行", "执行", "手动触发" | [run-task.md](references/run-task.md) |
| **Update Task** | "改成", "修改", "停用", "启用", "添加技�? | [update-task.md](references/update-task.md) |
...
- "3�?0�?.." (specific date - one-time task)
- "一次性任�?.." (one-time task)
- "提醒�?.." (remind me...)
- "早上/下午/晚上..." (morning/afternoon/evening...)
- "检�?运行/执行..." (check/run/execute...)
...
### 任务要求（`--prompt`�?
写入 `--prompt` 及任务卡片中�?**任务要求** 时，按以下规则处理：

1. **用户有明确表�?*：若用户对定时任务的任务要求给出了明确、可识别的写法（原句、列表、步骤、引用等），**严格按用户原话复�?*，不要改写、概括、删减或替换成自己的表述�?2. **用户无明确表�?*：若用户未单独说明任务要求，或表述过于笼统（仅说「提醒我」「做个日报」等），则根据对话上下文**尽可能准确、全面地**描述用户意图，形成可执行的任务要求；若使用技能、文件或 MCP，须在任务要求中一并体现�?
---
## Reference Documents
