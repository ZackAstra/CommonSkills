---
name: yunmai-disable
description: 使企业下发的 SASE/EDR/安全客户端（典型如 YunMai 云脉）静默、禁自启、停止后台驻留与自愈复活、解除其对 Windows 安全中心与 AMSI 的接管，并安全清理本机残留的可复用方法论。触发场景：此类客户端开机自启、进程常驻且删除后复活、退出/注销需密码、Windows 安全中心被其接管（病毒和威胁防护显示第三方产品并出现 Open app）、设置-应用列表中找不到而无法常规卸载、删除后仍有进程或重启复现；同时需要区分“本地静默隔离（改名保留、可回滚、不删除）”与“走 IT 合规注销（服务端解绑账号/设备、防止重新下发、满足合规审计）”两种模式的差异与配合。
metadata:
  version: 0.3.2
---

# YunMai disable —— 静默与禁用方法论（v0.3.2）
...
3. 自启动链路 = 0：登录 Run/RunOnce（含 32 位视图与当前用户）、启动文件夹、计划任务、StartupApproved、引导相关标记均无相关条目。
...
1 勘察（只读、全量）：进程与父子链 / 服务驱动注册 / 自启动点 / 安全中心与 AMSI /
   ...
   用户配置基线（先记录，后处理）；勘察工具类别见 §2.1，全程只读不修改。
...
flowchart TD
  ...
  C1 & C2 & C3 & C4 & C5 & C6 --> D[逐项复核验收，含一次重启与观察窗口]
  D --> E{复现?}
